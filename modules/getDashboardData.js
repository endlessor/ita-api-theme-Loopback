...
module.exports = () => {
  ...
  aggregates['totalSales'] = [{
      "$match": {
        "nMonth": {
          "$lte": parseInt(moment().format('YYYYMM')),
          "$gte": getLastMonth()
        },
        "status": "processed"
      }
  ...
  function getLastMonth() {
    var current = parseInt(moment().format('MM'));
    if (current == 1 || current == 2) {
      return parseInt(parseInt(parseInt(moment().format('YYYY')) - 1) + (current == 1 ? '12' : '11'));
    } else {
      return (parseInt(moment().format('YYYYMM')) - 2);
    }
  }
